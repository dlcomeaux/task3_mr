...
################################################################################
# Load data using MDT analysis data cleaning (stored in separate repo)
################################################################################
...
```
# Prep the data loaded from MDT analysis
...
chain_id <-
  ...
  mutate(chain_legs = n() - ifelse(min(placeno) == 1,1,0)) %>% 
  ungroup()
...
mode_share_base <-
  ...
  mutate(destination_place = case_when(
    ...
    # state_fips == 17 & county_fips == 31 ~ "Cook (excl. central area)",
  ...
  select(sampno,perno,weight,placeno,
         ...
         home_to_work,chain_id,chain_legs,chain_c,
         home_county_chi,
...
destination_alltrips_mode_share <-
  ...
  arrange(destination_place) # %>% View()

destination_worktrips_mode_share <-
  pct_calculator(mode_share_base %>%
                   # Keep only trips to work
                   filter(home_to_work == 1) %>% 
                   # Only keep the last leg of a chain to work to identify the
                   # actual destination
                   group_by(sampno,perno,chain_id) %>% 
                   mutate(max_placeno = max(placeno)) %>% 
                   ungroup() %>% 
                   filter(placeno == max_placeno),
                 breakdown_by = "mode_c",
                 second_breakdown = "destination_place",
                 weight = "weight") %>% 
  # mutate(pct = round(pct,2)) %>% 
...
## Chart of mode share, all trips, by destination
...
## Chart of work trips, mode share by destination
...
finalize_plot(p2,
              ...
              caption = "Note: The Central Area includes all trips to 
              tracts within the area bounded by North Ave., Ashland Ave., 
              Cermak Rd., and Lake Michigan. Includes only trips identified as a 
              journey from home to work. For trips to work that are part of 
              a larger 'chain' of trips, this analysis only evaluated the final 
              leg of the journey.
              <br><br>
              Source: Chicago Metropolitan Agency for Planning analysis of My 
              Daily Travel data.",
...
destination_worktrip_travel_stats <-
  ...
  filter(home_to_work == 1) %>%
  # Create binary for whether the trip has multiple legs
  mutate(one_leg = chain_legs == 1) %>%
